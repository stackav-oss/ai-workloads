...
class PDFSampler(Sampler):
    ...
    def generate_ray_samples(
        ...
        weights = weights[..., 0]
        self._assert_finite("pdf_sampler.weights", weights)
        ...
        self._assert_finite("pdf_sampler.pdf", pdf)
        cdf = torch.min(torch.ones_like(pdf), torch.cumsum(pdf, dim=-1))
        cdf = torch.cat(
            [torch.zeros_like(cdf[..., :1]), cdf], dim=-1
        )  # (num_rays, num_samples+1)
        self._assert_finite("pdf_sampler.cdf", cdf)
        ...
        self._assert_finite("pdf_sampler.u", u)
        self._assert_finite("pdf_sampler.existing_bins", existing_bins)
        ...
        self._assert_finite("pdf_sampler.bins", bins)
...
class NeuSSampler(Sampler):
    ...
    def __init__(
        ...
        train_stratified=True,
        single_jitter=True,
        num_samples_importance_prior=None,
        debug_numerics=False,
    ):
        super().__init__()
        self.num_samples = num_samples
        ...
        self.initial_sampler_name = initial_sampler
        self.debug_numerics = debug_numerics

        # samplers
        _sampler_map = {
            "UniformSampler": UniformSampler,
            "LinearDisparitySampler": LinearDisparitySampler,
            "PriorSampler": PriorSampler,
        }
        initial_sampler_kwargs = {
            "num_samples": num_samples,
            "train_stratified": train_stratified,
            "single_jitter": single_jitter,
        }
        if initial_sampler == "PriorSampler":
            initial_sampler_kwargs["num_samples_importance"] = (
                num_samples_importance_prior
            )
        self.initial_sampler = _sampler_map[initial_sampler](
            **initial_sampler_kwargs,
        )
    ...
    def generate_ray_samples(self, ray_bundle, sdf_fn, **kwargs):
        if self.initial_sampler_name == "PriorSampler":
            ray_samples = self.initial_sampler(ray_bundle, sdf_fn=sdf_fn, **kwargs)
        else:
        ...
        while total_iters < self.num_upsample_steps:
            ...
            if total_iters == 0:
                output_dict.update(
                    {"init_sampled_points": new_points, "init_weights": weights}
                )
    ...
    def generate_ray_samples(self, ray_bundle, density_fn, sdf_fn, **kwargs):
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            if total_iters == 0:
                output_dict.update(
                    {
                        "init_sampled_points": new_points,
...
class UniSurfSampler(Sampler):
    ...
    def generate_ray_samples(self, ray_bundle, occupancy_fn, sdf_fn, **kwargs):
        ...
        output_dict.update(
            {
                "init_sampled_points": ray_samples.frustums.get_start_positions(),
                "init_weights": weights,
            }
        )
...
class PriorSampler(Sampler):
    """Prior sampler that consider the sparse volume prior, but it's not originally sparse, remain to be improved."""

    def __init__(
        self,
        num_samples,
        num_samples_importance=None,
        train_stratified=True,
        single_jitter=True,
    ):
        super().__init__()
        self.num_samples = num_samples
        self.num_samples_importance = (
            48 if num_samples_importance is None else num_samples_importance
        )
        self.num_upsample_steps = 1

        # samplers
        self.initial_sampler = UniformSampler(
            num_samples=num_samples,
            train_stratified=train_stratified,
            single_jitter=single_jitter,
        )
    ...
    def generate_ray_samples(
        self, ray_bundle, sdf_fn, sdf_field, feature_volume, **kwargs
    ):
        occupancy_mask = kwargs.get("occupancy_mask", None)
        ...
        with torch.no_grad():
            ...
            if occupancy_mask is not None:
                # Use the pre-computed binary occupancy mask (1, D, H, W)
                # Treat it as a 1-channel feature volume and reuse sdf_field's interpolation
                occ_at_points = sdf_field.interpolate_feats(new_points, occupancy_mask)
                non_empty_points = occ_at_points.squeeze(-1) > 0.5
            else:
                # Fallback: use feature volume (original behavior)
                point_features = sdf_field.interpolate_feats(new_points, feature_volume)
                non_empty_points = point_features.any(-1)

        weights = torch.zeros(
            [*new_points.shape[:2], 1], dtype=new_points.dtype, device=new_points.device
        )
        weights[non_empty_points] = 1.0

        # Fallback: if a ray has NO occupied samples, use uniform weights
        ray_has_support = non_empty_points.any(dim=1)
        if not ray_has_support.all():
            weights[~ray_has_support] = 1.0
        ...
        output_dict.update({"ray_samples": new_samples})
        return output_dict["ray_samples"]
