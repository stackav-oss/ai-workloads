...
from mmdet3d_plugin._compat import auto_fp16
from torch import nn as nn
import torch
from mmdet3d_plugin._compat import SparseBasicBlock, make_sparse_convmodule
from mmdet3d_plugin._compat import spconv
from mmdet3d_plugin._compat import MIDDLE_ENCODERS
from ..utils import sparse_utils
from timm.models.layers import trunc_normal_


@MIDDLE_ENCODERS.register_module()
class MaskSparseEncoderHD(nn.Module):
    ...
    def __init__(
        ...
        if mae_cfg is not None and mae_cfg.get("learnable", False):
    ...
    def forward(self, voxel_features, coors, batch_size, return_encode_features=False):
        ...
        if self.mae_cfg is not None:
            block_coors = torch.cat(
                [
                    coors[:, :1],
                    torch.div(coors[:, 1:], self.mae_cfg.downsample_scale).int(),
                ],
                dim=-1,
            )
            block_coors, inverse_indices = block_coors.unique(
                sorted=False, return_inverse=True, dim=0
            )
            block_active = []
            for bs_idx in range(batch_size):
                block_active.append(
                    sparse_utils.random_masking(
                        1,
                        (block_coors[:, 0] == bs_idx).sum().item(),
                        1,
                        self.mae_cfg.mask_ratio,
                        coors.device,
                    ).squeeze()
                )
