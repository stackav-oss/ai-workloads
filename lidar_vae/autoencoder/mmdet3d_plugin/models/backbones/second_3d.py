...
from mmdet3d_plugin._compat import BaseModule
from torch import nn as nn

from mmdet3d_plugin._compat import BACKBONES
...
class SECOND3D(BaseModule):
    ...
    def __init__(
        ...
        self.kernel_type = (
            conv_cfg["type"] if isinstance(conv_cfg, dict) else conv_cfg.type
        )
        ...
        for i, layer_num in enumerate(layer_nums):
            block = [
                build_conv_layer(
                    ...
                    out_channels[i],
                    kernel,
                    stride=(1, layer_strides[i], layer_strides[i])
                    if len(padding) == 3
                    else (layer_strides[i], layer_strides[i]),
                    padding=padding,
                ),
    ...
    def forward(self, x):
        ...
        if self.kernel_type == "Conv2d":
            outs = [
                _out.reshape(batch, -1, *_out.shape[-3:]).transpose(1, 2)
                for _out in outs
            ]
