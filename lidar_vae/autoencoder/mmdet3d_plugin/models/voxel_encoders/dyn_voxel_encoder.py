...
import torch
from mmdet3d_plugin._compat import force_fp32
from torch import nn
import torch.nn.functional as F
from mmdet3d_plugin._compat import VOXEL_ENCODERS
import numpy as np
...
class CustomDynamicSimpleVFE(nn.Module):
    ...
    def forward(self, pts):
        ...
        # Pick one point per unique voxel (the last occurrence = max index per group)
        num_unique = inverse.max().item() + 1
        arange = torch.arange(len(coords), device=coords.device)
        indices = torch.zeros(num_unique, dtype=torch.long, device=coords.device)
        indices.scatter_(
            0, inverse, arange
        )  # last-write wins → picks last point per voxel
